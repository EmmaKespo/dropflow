...
export default function SuccessScreen({
  userType, generatedLink, customerPhone, riderPhone, onReset
}: SuccessScreenProps) {
  ...
  return (
    <div className="space-y-6 text-center py-2 animate-fade-in">
      <div className="space-y-1.5">
        <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-neutral-100 text-md">✅</div>
      ...
      <div className="space-y-2.5 text-left">
        <button 
          onClick={handleCopy}
          className="w-full border border-black bg-white text-black py-2.5 text-xs font-extrabold uppercase tracking-widest hover:bg-neutral-50 transition"
        >
          {isCopied ? "Copied Path Output!" : "📋 Copy Tracking Link"}
        </button>
        ...
        {userType === "free" && (
          <a 
            ...
            className="block w-full border border-black bg-black text-white py-2.5 text-xs font-extrabold uppercase tracking-widest hover:bg-neutral-900 transition text-center"
          >
        ...
        <a 
          ...
          target="_blank" rel="noopener noreferrer"
          className="block w-full border border-neutral-300 bg-white text-neutral-500 py-2.5 text-xs font-extrabold uppercase tracking-widest hover:text-black hover:border-black transition text-center"
        >
        ...
        {userType === "premium" && (
          <button 
            onClick={onReset}
            className="w-full border border-black bg-black text-white py-2.5 text-xs font-extrabold uppercase tracking-widest hover:bg-neutral-900 transition text-center"
          >
            👁️ View Delivery Log Metrics
          </button>
        )}
      </div>

      <button onClick={onReset} className="text-[10px] font-black uppercase tracking-widest text-neutral-400 hover:text-black underline transition mt-2">
